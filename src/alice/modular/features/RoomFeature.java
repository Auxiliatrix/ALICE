...
public class RoomFeature extends MessageFeature {
	...
	protected class RoomPassiveFeature extends Feature<VoiceStateUpdateEvent> {
		...
		protected Mono<?> respond(VoiceStateUpdateEvent type) {
			...
			MultipleDependentStacker stacker = new MultipleDependentStacker(type.getCurrent().getChannel(), type.getOld().isPresent() ? type.getOld().get().getChannel() : Mono.empty(), type.getCurrent().getGuild(), type.getCurrent().getMember(), type.getOld().isPresent() ? type.getOld().get().getChannel().map(c -> c.getVoiceStates().collectList()) : Mono.empty());
