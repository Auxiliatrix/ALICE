...
public class BlackboxModule extends MessageModule {
	...
	public Command<MessageCreateEvent> buildCommand(Builder<MessageCreateEvent> dfb) {
		...
		open.withDependentEffect(mcdm.buildEffect(
			(mce, mc) -> {
				SyncedJSONObject ssf = SaveFiles.ofGuild(mce.getGuildId().get());
				SyncedJSONObject blackbox = ssf.getJSONObject("%blackbox");
				if( blackbox.has(mc.getId().asString()) ) {
		...
		close.withDependentEffect(mcdm.buildEffect(
			(mce, mc) -> {
				SyncedJSONObject ssf = SaveFiles.ofGuild(mce.getGuildId().get());
				SyncedJSONObject blackbox = ssf.getJSONObject("%blackbox");
				if( blackbox.has(mc.getId().asString()) ) {
					return mc.getMessagesAfter(Snowflake.of(blackbox.getString(mc.getId().asString()))).takeWhile(m -> m.getId().compareTo(mce.getMessage().getId()) <= 0)
		...
		cancel.withDependentEffect(mcdm.buildEffect(
			(mce, mc) -> {
				SyncedJSONObject ssf = SaveFiles.ofGuild(mce.getGuildId().get());
				SyncedJSONObject blackbox = ssf.getJSONObject("%blackbox");
				if( blackbox.has(mc.getId().asString()) ) {
