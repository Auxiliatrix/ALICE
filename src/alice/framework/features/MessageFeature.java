...
public abstract class MessageFeature extends Feature<MessageCreateEvent> {
	...
	protected boolean listen(MessageCreateEvent event) {
		return (!checkPublic && (event.getMember().isPresent() && isAllowed(event.getMember().get()) || !event.getMember().isPresent()) || event.getMember().isPresent() && isAllowed(event.getMember().get()))
